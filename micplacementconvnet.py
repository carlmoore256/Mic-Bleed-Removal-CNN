...
class callback(keras.callbacks.Callback):
    def __init__(self, x_val, y_val, model, num_tests=1):
        self.losses = []
        self.model = model
        self.x_val = x_val
        self.y_val = y_val
...
def extract_transients(audio, sr, ws, start_pad, hop=512):
  # grab onset times; backtrack detects minimum before transient
  beats = librosa.onset.onset_detect(y=audio, sr=sr, units='frames', hop_length=hop, backtrack=True)
  ...
  for i, k in enumerate(data.keys()):
    ...
    try:
      if len(x) > 0 and len(y) > 0:
        ...
        print(f'loading {data[k][x_key][0]}, {i} of {max_examples}')
    ...
    if i > max_examples: # HARD CODED STOP FOR TESTING, REMOVE ME
      break
...
x_train, y_train = gen_dataset(data, win_size, x_key, y_key, 
                               normalize_stems=False, normalize_transients=True, 
                               max_examples=150)
...
for i in range(10):
  ...
  plt.plot(x_train[rand_idx, :])
  plt.plot(y_train[rand_idx, :])
  plt.show()

# ==== PREPARE DATASET DIMENSIONS ========

val_examps = 1000 # number of validation samples (takes from training)
...
model.save('/content/drive/My Drive/Datasets/MultitrackStems/models/oh_to_snare_10_epoch.h5')
...
audio_file = '/content/OverheadSnippet.wav'
sample_rate = 44100
win_size = 16384
